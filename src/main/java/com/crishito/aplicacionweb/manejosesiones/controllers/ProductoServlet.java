...
public class ProductoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws
       ...
       /* try (PrintWriter out =
            ...
            // condicional con alerta Bootstrap
            ...
            productos.forEach(p -> {
                ...
                out.println(" <td>" +
                        p.getIdProducto() + "</td>");
                out.println("<td>" + p.getNombre() +
                        "</td>");
                // Se usa p.getCategoria().getNombre() para obtener el nombre de la categoría
                out.println(" <td>" +
                        p.getCategoria().getNombre() + "</td>");

                // Columnas de datos
                // condicionales
                if (usernameOptional.isPresent())
                {
                    out.println("<td>$" + String.format("%.2f",
                            p.getPrecio()) + "</td>");

                    // **BOTÓN 'AGREGAR'
                    // RESTAURADO (SOLO ICONO)**
