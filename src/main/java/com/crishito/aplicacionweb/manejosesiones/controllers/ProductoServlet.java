...
public class ProductoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // Usando los nombres de clases corregidos
        ProductoService service = new ProductoServiceImpl();
        List<Producto> productos = service.listar();

        LoginService auth = new LoginServiceSessionImpl(); // Nombre de clase corregido
        ...
        try (PrintWriter out = resp.getWriter()) {
            ...
            // Enlaces de Bootstrap y Bootstrap Icons (Íconos)
            ...
            // Usando el estilo de fondo consistente
            ...
            if (usernameOptional.isPresent()) {
                ...
                out.println("  <div class=\"alert alert-warning text-center\" role=\"alert\">Inicia sesión para ver los precios y agregar productos al carro.</div>");
            }
            ...
            if (usernameOptional.isPresent()) {
                ...
                out.println("        <th><i class=\"bi bi-cart-plus\"></i> Opciones</th>");
            }
            out.println("      </tr>");
            out.println("    </thead>");
            out.println("    <tbody>");
            ...
            productos.forEach(p -> {
                out.println("    <tr>");
                // Usar getIdProducto() en lugar de getId()
                out.println("      <td>" + p.getIdProducto() + "</td>");
                out.println("      <td>" + p.getNombre() + "</td>");
                // Cambié getTipo() a getCategoria() si tu modelo lo usa, si no, déjalo como getTipo()
                ...
                if (usernameOptional.isPresent()) {
                    ...
                    // Columna de opciones (Agregar al Carro)
                    out.println("      <td><a href=\""
                            + req.getContextPath()
                            + "/agregar-carro?id="
                            //Usar getIdProducto() en el enlace
                            + p.getIdProducto()
                            + "\" class=\"btn btn-sm btn-success\"><i class=\"bi bi-cart-plus\"></i> Agregar</a></td>");
                }
                out.println("    </tr>");
            });
            ...
            out.println("</div>"); // Cierre del div card
            out.println("</div>"); // Cierre del div container
            out.println("</body>");
            out.println("</html>");
        }
    }
